...
def open_add_dialog():
    ...
    with ui.dialog() as dialog, ui.card().classes("min-w-[600px] max-w-[800px] p-6"):
        # Заголовок диалога
        with ui.row().classes("items-center gap-2 mb-4"):
            ui.icon("add_circle").classes("text-xl text-green-600")
            ui.label("Добавить новую сделку").classes("text-xl font-bold text-gray-800")
        ...
        with ui.row().classes("justify-end gap-3 mt-6"):
            ...
            def save_transaction():
                try:
                    ...
                    if "refresh" in globals():
                        refresh()
                except Exception as e:
                    ui.notify(f"❌ Ошибка: {e}", type="negative")

            ui.button("Добавить сделку", on_click=save_transaction).props(
                "color=primary"
            ).classes("px-4 py-2")
...
def portfolio_page():
    ...
    with ui.row().classes("w-full h-screen overflow-hidden"):
        ...
        # ОСНОВНОЙ КОНТЕНТ
        with ui.column().classes("flex-1 bg-gray-50 overflow-y-auto"):
                ...
                with ui.row().classes("items-center justify-between"):
                    ...
                    with ui.row().classes("gap-3"):
                        ...
                        ui.button("+ Добавить сделку", icon="add").classes(
                            "bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-lg font-semibold shadow-lg transition-all duration-200 transform hover:scale-105"
                        ).on("click", lambda: open_add_dialog())

                        # Быстрые действия
                        with ui.button("⚡ Быстрые действия", icon="flash_on").classes(
                            "bg-orange-500 hover:bg-orange-600 text-white px-4 py-3 rounded-lg font-semibold shadow-lg transition-all duration-200"
                        ):
                            with ui.menu():
                                ui.menu_item(
                                    "💰 Получить текущую цену",
                                    on_click=lambda: ui.notify(
                                        "Функция в разработке", type="info"
                                    ),
                                )
                                ui.menu_item(
                                    "📊 Экспорт данных",
                        ...
                        refresh_button = (
                            ...
                            .classes(
                                "bg-white/20 hover:bg-white/30 text-white border-white/30 backdrop-blur-sm transition-all duration-200 px-4 py-2"
                            )
                            .on("click", lambda: refresh())
                        )
            ...
            # Компактные фильтры (свернуты по умолчанию)
            with ui.card().classes("p-4 mb-6 bg-gray-50 border border-gray-200"):
                with ui.row().classes("items-center justify-between"):
                    ...
                    filter_toggle = ui.button("Показать", icon="expand_more").classes(
                        "bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-1 rounded text-sm"
                    )

                # Свернутая область фильтров
                with ui.column().classes("gap-3 mt-3"):
                    with ui.row().classes("gap-4 items-end"):
                        coin_filter = (
                            ui.input(placeholder="Монета (BTC, ETH...)", label="Монета")
                            .props("uppercase autocomplete=off")
                            .classes("w-40")
                        )

                        strat_filter = ui.select(
                            ["(все)"] + STRATS, value="(все)", label="Стратегия"
                        ).classes("w-32")

                        type_filter = ui.select(
                            ["(все)"] + TYPES, value="(все)", label="Тип"
                        ).classes("w-32")

                        period_filter = ui.select(
                            ["(все)", "Сегодня", "Неделя", "Месяц"],
                            value="(все)",
                            label="Период",
                        ).classes("w-28")
                ...
                with ui.row().classes("justify-end gap-2"):
                    ui.button("Сбросить", icon="clear", on_click=reset_filters).classes(
                        "bg-gray-200 hover:bg-gray-300 text-gray-700 px-3 py-1 rounded text-sm"
                    )
            ...
            # Вкладки с приоритизацией (самые важные слева)
