...
from app.core.models import PriceAlertIn, TransactionIn
from app.core.services import (  # Алерты
    add_price_alert,
    add_transaction,
    check_price_alerts,
    delete_price_alert,
    ...
    with ui.dialog() as dialog, ui.card().classes("min-w-[700px] max-w-[900px] p-6"):
        ...
        def update_quick_source_buttons(top_sources):
            ...
            try:
                if quick_buttons_container:
                    ...
                    if top_sources:
                        ui.label("Популярные источники:").classes(
        ...
        def edit_source_name(old_name, mgmt_dialog, refresh_func):
            ...
            with ui.dialog() as edit_dialog, ui.card().classes("p-6"):
                ui.label(f"Редактировать: {old_name}").classes(
                    "text-lg font-semibold mb-4"
                )

                new_name_input = ui.input("Новое название", value=old_name).classes(
                    "w-full mb-4"
                )
                ...
                def save_changes():
                    ...
                    if success:
                        ui.notify(
                            f"Источник переименован: {old_name} -> {new_name}",
                            type="positive",
                        )
                        edit_dialog.close()
                ...
                with ui.row().classes("justify-end gap-3"):
                    ...
                    ui.button("Сохранить", on_click=save_changes).classes(
        ...
        def delete_source(source_name, mgmt_dialog, refresh_func):
            ...
            with ui.dialog() as delete_dialog, ui.card().classes("p-6"):
                ui.label(f"Удалить источник '{source_name}'?").classes(
                    "text-lg font-semibold mb-4"
                )
                ui.label("Это удалит источник из всех сделок.").classes(
        ...
        def move_source_up(source_name, mgmt_dialog, refresh_func):
            ...
            if success:
                ...
                ui.notify(
                    f"Не удалось переместить '{source_name}' вверх", type="negative"
                )
        ...
        def add_new_source(mgmt_dialog, refresh_func):
            ...
            with ui.dialog() as add_dialog, ui.card().classes("p-6"):
                ...
                def add_source():
                    ...
                    ui.notify(
                        f"Источник '{source_name}' готов к использованию",
                        type="positive",
                    )
                    add_dialog.close()
                    refresh_func()
        ...
        def open_sources_management_dialog():
                ...
                if sources_container:
                    ...
                    for i, (source_name, frequency) in enumerate(sources_with_freq):
                        with sources_container:
                            with ui.row().classes(
                                "items-center gap-3 p-3 bg-gray-50 rounded-lg"
                            ):
                                # Номер по популярности
                                ui.label(f"{i+1}.").classes(
                                    "text-sm font-bold text-gray-600 w-8"
                                )

                                # Название источника
                                ui.label(source_name).classes(
                                    "flex-1 text-sm font-medium"
                                )

                                # Частота использования
                                ui.label(f"({frequency} раз)").classes(
                                ...
                                with ui.row().classes("gap-1"):
                                    ui.button(
                                        "✏️",
                                        on_click=lambda s=source_name: edit_source_name(
                                            s, mgmt_dialog, refresh_both
                                        ),
                                    ).props("size=sm flat").classes("text-blue-600")
                                    ui.button(
                                        "🗑️",
                                        on_click=lambda s=source_name: delete_source(
                                            s, mgmt_dialog, refresh_both
                                        ),
                                    ...
                                    if i > 0:
                                        ui.button(
                                            "⬆️",
                                            on_click=lambda s=source_name: move_source_up(
                                                s, mgmt_dialog, refresh_both
                                            ),
                                        ).props("size=sm flat").classes(
                                            "text-green-600"
                                        )
                                    if i < len(sources_with_freq) - 1:
                                        ui.button(
                                            "⬇️",
                                            on_click=lambda s=source_name: move_source_down(
                                                s, mgmt_dialog, refresh_both
                                            ),
                                        ).props("size=sm flat").classes(
                                            "text-green-600"
            ...
            with mgmt_dialog, ui.card().classes("min-w-[600px] max-w-[800px] p-6"):
                # Заголовок
                with ui.row().classes(
                    "items-center gap-2 mb-6 pb-4 border-b border-gray-200"
                ):
                    ui.icon("settings").classes("text-2xl text-blue-600")
                    ui.label("Управление источниками").classes(
                        "text-2xl font-bold text-gray-800"
                    )

                # Список источников с возможностью редактирования
                with ui.column().classes("space-y-3"):
                    ui.label("Источники (сортировка по популярности):").classes(
                ...
                with ui.row().classes(
                    ...
                    ui.button(
                        "Добавить источник",
                        on_click=lambda: add_new_source(mgmt_dialog, refresh_both),
                    ).classes(
                        "bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg"
                    )
        ...
        def show_sources_statistics():
            ...
            with ui.dialog() as stats_dialog, ui.card().classes(
                "min-w-[500px] max-w-[700px] p-6"
            ):
                # Заголовок
                with ui.row().classes(
                    "items-center gap-2 mb-6 pb-4 border-b border-gray-200"
                    ...
                    ui.label("Статистика источников").classes(
                        "text-2xl font-bold text-gray-800"
                    )
                ...
                with ui.card().classes("p-4 bg-blue-50 mb-4"):
                    ui.label("Общая статистика").classes(
                        "text-lg font-semibold text-blue-800 mb-2"
                    )
                    ui.label(f"Всего сделок: {total_transactions}").classes(
                        "text-sm text-blue-700"
                    )
                    ui.label(f"Уникальных источников: {unique_sources}").classes(
                        "text-sm text-blue-700"
                    )

                # Топ источников
                with ui.column().classes("space-y-2"):
                    ui.label("Топ источников:").classes(
                        "text-sm font-medium text-gray-700"
                    )
                    ...
                    if top_sources:
                        for i, (source_name, frequency) in enumerate(top_sources):
                            if frequency > 0:
                                percentage = (
                                    (frequency / total_transactions * 100)
                                    if total_transactions > 0
                                    else 0
                                )
                                with ui.row().classes(
                                    "items-center gap-3 p-2 bg-gray-50 rounded"
                                ):
                                    ui.label(f"{i+1}.").classes(
                                        "text-sm font-bold text-gray-600 w-8"
                                    )
                                    ...
                                    with ui.column().classes("flex-1"):
                                        ui.linear_progress(percentage / 100).classes(
                                            "h-2"
                                        )
                    else:
                        ui.label("Нет данных о сделках").classes(
        ...
        def get_current_price():
            ...
            try:
                ...
                if price_data and price_data["price"]:
                    ...
                    message = f"✅ Средняя цена {coin_symbol}: ${current_price:,.2f}"

                    # Добавляем информацию об источниках
                    if source_count > 1:
                        message += (
                            f" (из {source_count} источников: {', '.join(sources)})"
                        )
                    else:
                    ...
                    if "price_range" in price_data:
                        ...
                        if spread > 0:
                            ...
                            message += (
                                f" [разброс: ${spread:.2f} ({spread_percent:.1f}%)]"
                            )
        ...
        with ui.grid(columns=2).classes("gap-6"):
            ...
            with ui.column().classes("gap-4"):
                ...
                with ui.column().classes("gap-1"):
                    render_required_label(
                        "💰 Монета",
                        "Введите символ криптовалюты или используйте кнопки",
                    )
                    ...
                    with ui.row().classes("gap-2 mt-2 flex-wrap"):
                        ui.label("Популярные:").classes(
                            "text-xs text-blue-600 font-medium self-center"
                        )
                        ...
                        try:
                            ...
                            for tx in transactions:
                                ...
                                if coin_symbol:
                                    coin_counts[coin_symbol] = (
                                        coin_counts.get(coin_symbol, 0) + 1
                                    )
                ...
                with ui.column().classes("gap-1"):
                    render_required_label(
                        "📊 Тип операции", "buy = покупка, sell = продажа"
                    )
                    ttype = ui.select(TYPES, label="Выберите тип", value="buy").classes(
                        "w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    )
                ...
                with ui.column().classes("gap-1"):
                    with ui.row().classes("items-center gap-2"):
                        ...
                        ui.button("📊 Текущая цена", on_click=get_current_price).props(
                            "size=sm outline"
                        ).classes(
                            "text-xs bg-blue-100 hover:bg-blue-200 text-blue-700 px-2 py-1 rounded"
                        ).tooltip(
            ...
            with ui.column().classes("gap-4"):
                ...
                with ui.column().classes("gap-1"):
                    render_required_label(
                        "🎯 Стратегия", "long = долгосрочная, short = краткосрочная"
                    )
                    strategy = ui.select(
                ...
                with ui.column().classes("gap-1"):
                    ui.label("🏢 Источник").classes("text-sm font-medium text-gray-700")
                    ...
                    all_sources = [source for source, freq in sources_with_freq]
                    top_sources = [
                        source for source, freq in sources_with_freq[:3]
                    ]  # Топ-3 для кнопок
                    ...
                    with ui.row().classes("gap-2 mt-2"):
                        ui.button("⚙️ Управление", icon="settings").props(
                            "size=sm outline"
                        ).classes(
                            "text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-2 py-1 rounded"
                        ).on(
                            "click", lambda: open_sources_management_dialog()
                        )
                        ui.button("📊 Статистика", icon="analytics").props(
                            "size=sm outline"
                        ).classes(
                            "text-xs bg-green-100 hover:bg-green-200 text-green-700 px-2 py-1 rounded"
                        ).on(
                            "click", lambda: show_sources_statistics()
                        )

                    ui.label(
                        "Выберите из списка, используйте кнопки или введите новый"
                    ).classes("text-xs text-gray-500")

                # Заметки
                with ui.column().classes("gap-1"):
                    ui.label("📝 Заметки").classes("text-sm font-medium text-gray-700")
                    notes = (
...
def create_overview_tab():
    ...
    with ui.column().classes("w-full h-full p-6 space-y-6"):
        ...
        with ui.row().classes("gap-4 flex-1"):
            ...
            with ui.column().classes("flex-1"):
                with ui.card().classes(
                    "p-4 bg-white shadow-sm rounded-lg border border-gray-200 h-full"
                ):
                    ui.label("Топ позиции").classes(
                        "text-lg font-semibold text-gray-800 mb-2"
                    )
                    top_positions_container = ui.column().classes(
                        "space-y-1 max-h-[600px] overflow-y-auto"
                    )
            ...
            with ui.column().classes("flex-1"):
                with ui.card().classes(
                    ...
                    ui.label("Худшие позиции").classes(
                        "text-lg font-semibold text-gray-800 mb-2"
                    )
                    worst_positions_container = ui.column().classes(
                        "space-y-1 max-h-[600px] overflow-y-auto"
                    )
            ...
            with ui.column().classes("flex-1"):
                with ui.card().classes(
                    "p-6 bg-white shadow-sm rounded-lg border border-gray-200 h-full"
                ):
                    ui.label("Стоимость портфеля").classes(
                        "text-lg font-semibold text-gray-800 mb-4"
                    )
                    portfolio_chart_container = ui.column().classes(
                ...
                try:
                    ...
                    if top_positions:
                        for i, pos in enumerate(
                            ...
                            with ui.card().classes(
                                "p-1 bg-gray-50 hover:bg-gray-100 transition-colors"
                            ):
                                with ui.row().classes("items-center justify-between"):
                                    with ui.row().classes("items-center gap-1"):
                                        ui.label(f"#{i}").classes(
                                            "text-xs text-gray-500 w-4 font-medium"
                                        )
                                        ui.label(f"{coin}").classes(
                                    ...
                                    with ui.column().classes("text-right"):
                                        ...
                                        ui.label(f"{pnl:+.2f}").classes(
                                            f"text-xs font-medium {pnl_color}"
                                        )
                    else:
                ...
                try:
                    ...
                    if positions:
                        ...
                        worst_positions = sorted(
                            positions, key=lambda x: x.get("unreal_pnl", 0)
                        )[:5]
                        ...
                        for i, pos in enumerate(worst_positions, 1):
                            ...
                            with ui.card().classes(
                                "p-1 bg-red-50 hover:bg-red-100 transition-colors"
                            ):
                                with ui.row().classes("items-center justify-between"):
                                    with ui.row().classes("items-center gap-1"):
                                        ui.label(f"#{i}").classes(
        ...
        def refresh_portfolio_chart():
            ...
            with portfolio_chart_container:
                try:
                    portfolio_stats = get_portfolio_stats()
                    total_value = portfolio_stats.get("totals", {}).get(
                        "total_value", 0
                    )
                    total_unreal = portfolio_stats.get("totals", {}).get(
                        "total_unreal", 0
                    )
                    ...
                    with ui.column().classes("items-center space-y-4"):
                        ...
                        # Стоимость портфеля
                        ui.label(f"${total_value:.2f}").classes(
                        ...
                        if total_unreal >= 0:
                            ...
                            ui.label(f"+${total_unreal:.2f}").classes(
                        ...
                        ui.label("Общая стоимость портфеля").classes(
        ...
        def refresh_overview_data():
            ...
            with stats_container:
                try:
                    ...
                    daily_pnl = (
                        total_unreal  # В будущем можно добавить расчет дневного PnL
                    )
                    ...
                    create_compact_stat_card(
                        "Дневной PnL", f"{daily_pnl:+.2f} USD", "📈", daily_pnl >= 0
                    )
                    create_compact_stat_card(
                        "Нереализованный PnL",
                        f"{total_unreal:+.2f} USD",
                        "💎",
                        total_unreal >= 0,
                    )
                    create_compact_stat_card(
                        "Реализованный PnL",
                        f"{total_realized:+.2f} USD",
                        "✅",
                        total_realized >= 0,
                    )
...
def create_compact_stat_card(title, value, icon, is_positive=True):
    ...
    with ui.card().classes(
        "p-4 bg-white shadow-sm rounded-lg border border-gray-200 flex-1 hover:shadow-md transition-shadow"
    ):
        ...
        with ui.column().classes("flex-1 bg-gray-50 overflow-hidden"):
            ...
            with ui.row().classes(
                ...
                with ui.row().classes("items-center space-x-3"):
                    ...
                    with ui.button(icon="notifications", color="primary") as badge_btn:
                        notification_badge = ui.badge("0", color="red").classes(
                    ...
                    ui.button("⚙️ Настройки", icon="settings").classes(
                        ...
                        "click",
                        lambda: ui.notify("Настройки в разработке", color="info"),
                    )
                    ...
                    ui.button("❓ Помощь", icon="help").classes(
                        ...
                        "click", lambda: ui.notify("Справка в разработке", color="info")
                    )

                    # Индикатор статуса (уникальная для верхней панели)
                    with ui.row().classes(
                        "items-center gap-2 px-3 py-2 bg-green-50 rounded-lg border border-green-200"
                    ):
            ...
            with ui.column().classes("flex-1 p-6"):
                ...
                def create_positions_tab():
                    with ui.column().classes(
                        "w-full p-4 max-h-[calc(100vh-200px)] overflow-y-auto"
                    ):
                        ui.label("🪙 Позиции портфеля").classes(
                        ...
                        with ui.row().classes("gap-3 mb-4"):
                            ...
                            ).on("click", lambda: refresh_positions_data())
                            ui.button("📊 Аналитика", icon="analytics").classes(
                        ...
                        def refresh_positions_data():
                            ...
                            with positions_container:
                                try:
                                    ...
                                    if not positions:
                                        with ui.card().classes(
                                            "p-6 text-center bg-gray-50"
                                        ):
                                            ui.icon("inbox").classes(
                                                "text-4xl text-gray-400 mb-2"
                                            )
                                            ui.label("Нет открытых позиций").classes(
                                                "text-lg text-gray-500"
                                            )
                                            ui.label(
                                                "Добавьте сделки для создания позиций"
                                    ...
                                    with ui.card().classes(
                                        ...
                                        # Заголовок таблицы
                                        with ui.row().classes(
                                            "w-full bg-gray-100 p-3 rounded-t-lg font-semibold text-gray-700"
                                        ):
                                        ...
                                        for pos in positions:
                                            ...
                                            with ui.row().classes(
                                                f"w-full p-3 border-b border-gray-200 hover:bg-gray-50 {pnl_bg}"
                                            ):
                                                # Монета
                                                with ui.column().classes("flex-1"):
                                                    ui.label(coin).classes(
                                                ...
                                                # Количество
                                                ui.label(f"{qty:.4f}").classes(
                                                    "flex-1 text-gray-700"
                                                )

                                                # Средняя цена
                                                ui.label(f"${avg_cost:.2f}").classes(
                                                    "flex-1 text-gray-700"
                                                )
                                                ...
                                                if current_price > 0:
                                                    ...
                                                    ui.label("Загрузка...").classes(
                                                        "flex-1 text-gray-500 italic"
                                                    )
                                                ...
                                                if current_value > 0:
                                                    ui.label(
                                                        ...
                                                        "flex-1 font-semibold text-gray-800"
                                                    )
                                                else:
                                                    ui.label("Загрузка...").classes(
                                                        "flex-1 text-gray-500 italic"
                                                ...
                                                if current_price > 0:
                                                    ui.label(f"${pnl:.2f}").classes(
                                                        f"flex-1 font-semibold {pnl_color}"
                                                    )
                                                else:
                                                    ui.label("Загрузка...").classes(
                                                        "flex-1 text-gray-500 italic"
                                    ...
                                    totals = portfolio_stats.get("totals", {})
                                    with ui.card().classes(
                                        "p-6 bg-gradient-to-r from-blue-50 to-green-50 mt-4 min-h-[160px] mb-4"
                                    ):
                                        ui.label("📊 Сводка позиций").classes(
                                            "text-lg font-semibold text-blue-800 mb-4"
                                        )
                                        ...
                                        with ui.row().classes("w-full gap-6"):
                                            ...
                                            with ui.column().classes(
                                                ...
                                                ui.label("Нереализованный P&L").classes(
                                                    "text-sm text-gray-500 mb-2"
                                                )
                                                total_pnl = totals.get(
                                                    "total_unreal", 0
                                                )
                                                pnl_color = (
                                                    "text-green-600"
                                                ...
                                                ui.label(f"${total_pnl:.2f}").classes(
                                                    f"text-xl font-bold {pnl_color}"
                                                )

                                            with ui.column().classes(
                                                "flex-1 text-center min-w-[100px]"
                                            ):
                                                ui.label("ROI").classes(
                                                    "text-sm text-gray-500 mb-2"
                                                )
                                                ...
                                                ui.label(f"{roi:.1f}%").classes(
                                                    f"text-xl font-bold {roi_color}"
                                                )

                                            with ui.column().classes(
                                                "flex-1 text-center min-w-[100px]"
                                            ):
                                                ui.label("Позиций").classes(
                                                    "text-sm text-gray-500 mb-2"
                                                )
                                                ui.label(f"{len(positions)}").classes(
                                    ...
                                    with ui.card().classes("p-6 text-center bg-red-50"):
                                        ui.icon("error").classes(
                                            "text-4xl text-red-400 mb-2"
                                        )
                                        ui.label("Ошибка загрузки позиций").classes(
                                            "text-lg text-red-600"
                                        )
                                        ui.label(f"Детали: {e}").classes(
                ...
                def create_transactions_tab():
                    with ui.column().classes(
                        "w-full space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto p-4"
                    ):
                        ...
                        with ui.card().classes("p-4 bg-white shadow-sm rounded-lg"):
                            ...
                            try:
                                ...
                                if transactions:
                                    ...
                                    ui.table(
                                        columns=columns, rows=transactions, row_key="id"
                                    ).classes("w-full")
                                else:
                                    with ui.row().classes(
                                        "h-48 items-center justify-center bg-gray-50 rounded-lg"
                                    ):
                                ...
                                with ui.row().classes(
                                    ...
                                    ui.label(f"Ошибка загрузки: {e}").classes(
                                        "text-red-500"
                                    )
...
def create_alerts_tab():
    ...
    with ui.column().classes(
        ...
        # Кнопки управления
        with ui.row().classes("gap-3 mb-4"):
            ui.button("➕ Добавить алерт", icon="add").classes(
                "bg-blue-500 text-white"
            ).on("click", lambda: open_add_alert_dialog())
            ui.button("🔄 Проверить алерты", icon="refresh").classes(
                "bg-green-500 text-white"
            ).on("click", lambda: check_alerts())
            ui.button("📊 Статистика", icon="analytics").classes(
                "bg-purple-500 text-white"
            ).on("click", lambda: show_alert_statistics())

        # Список алертов
        with ui.card().classes("p-4 bg-white shadow-sm rounded-lg"):
            ui.label("Активные алерты").classes(
                "text-lg font-semibold text-gray-800 mb-4"
            )
                    ...
                    if alerts:
                        for alert in alerts:
                            with alerts_container:
                                with ui.card().classes(
                                    "p-3 mb-2 border-l-4 border-blue-400"
                                ):
                                    with ui.row().classes(
                                        "items-center justify-between"
                                    ):
                                        with ui.column().classes("flex-1"):
                                            ui.label(f"💰 {alert.coin}").classes(
                                                "font-semibold text-gray-800"
                                            )
                                            ui.label(
                                                f"Цель: {alert.target_price} {CURRENCY} ({alert.alert_type})"
                                            ).classes("text-sm text-gray-600")
                                            if alert.notes:
                                                ui.label(
                                                    f"Заметка: {alert.notes}"
                                                ).classes("text-xs text-gray-500")
                                        with ui.row().classes("gap-2"):
                                            ui.button(
                                                "✏️",
                                                on_click=lambda a=alert: edit_alert(a),
                                            ).props("size=sm flat").classes(
                                                "text-blue-600"
                                            )
                                            ui.button(
                                                "🗑️",
                                                on_click=lambda a=alert: delete_alert(
                                                    a
                                                ),
                                            ).props("size=sm flat").classes(
                                                "text-red-600"
                        ...
                        with alerts_container:
                            with ui.row().classes(
                                "h-32 items-center justify-center bg-gray-50 rounded-lg"
                            ):
                                ui.label("Нет активных алертов").classes(
                                    "text-gray-500"
                                )
                except Exception as e:
                    with alerts_container:
            ...
            def open_add_alert_dialog():
                ...
                with ui.dialog() as dialog, ui.card().classes("p-6 w-96"):
                    ui.label("Добавить алерт по цене").classes(
                        "text-lg font-semibold mb-4"
                    )

                    coin_input = ui.input("Монета (например, BTC)").classes(
                        "w-full mb-3"
                    )
                    ...
                    with ui.row().classes("w-full mb-3"):
                        ...
                        alert_type_select = ui.select(
                            ["above", "below"], value="above", label="Выберите тип"
                        ).classes("w-full")

                    notes_input = ui.textarea("Заметки (необязательно)").classes(
                        "w-full mb-4"
                    )
                    ...
                    def add_alert():
                        try:
                            ...
                            notes = (
                                notes_input.value.strip() if notes_input.value else None
                            )

                            if not coin or target_price <= 0:
                                ui.notify(
                                    "Заполните все обязательные поля", type="negative"
                                )
                                return
                    ...
                    with ui.row().classes("justify-end gap-3"):
                        ...
                        ui.button("Добавить", on_click=add_alert).classes(
                            "bg-blue-500 text-white"
                        )
            ...
            def show_alert_statistics():
                ...
                try:
                    ...
                    with ui.dialog() as dialog, ui.card().classes("p-6"):
                        ui.label("📊 Статистика алертов").classes(
                            "text-lg font-semibold mb-4"
                        )
...
def create_charts_tab():
    ...
    with ui.column().classes("w-full space-y-6"):
        ui.label("📈 Графики и визуализация").classes(
            "text-2xl font-bold text-gray-800"
        )

        # Кнопки управления
        with ui.row().classes("gap-3 mb-4"):
            ui.button("🔄 Обновить графики", icon="refresh").classes(
                "bg-blue-500 text-white"
            ).on("click", lambda: refresh_all_charts())
            ui.button("📊 Сводка", icon="analytics").classes(
                "bg-green-500 text-white"
            ).on("click", lambda: show_portfolio_summary())
        ...
        with ui.row().classes("w-full gap-4 mb-6"):
            ...
            with ui.card().classes("p-4 bg-blue-50 border-l-4 border-blue-400"):
                ...
                ui.label(str(summary["total_transactions"])).classes(
            ...
            with ui.card().classes("p-4 bg-green-50 border-l-4 border-green-400"):
                ...
                ui.label(str(summary["unique_coins"])).classes(
            ...
            with ui.card().classes("p-4 bg-purple-50 border-l-4 border-purple-400"):
                ...
                ui.label(str(summary["active_positions"])).classes(
            ...
            with ui.card().classes("p-4 bg-orange-50 border-l-4 border-orange-400"):
                ...
                ui.label(f"{summary['total_volume']:.2f} {CURRENCY}").classes(
        ...
        with ui.row().classes("w-full gap-4"):
            ...
            with ui.column().classes("flex-1 space-y-4"):
                ...
                with ui.card().classes("p-4 bg-white shadow-sm rounded-lg"):
                    ui.label("Распределение портфеля").classes(
                        "text-lg font-semibold text-gray-800 mb-4"
                    )
                ...
                with ui.card().classes("p-4 bg-white shadow-sm rounded-lg"):
                    ui.label("Временная линия транзакций").classes(
                        "text-lg font-semibold text-gray-800 mb-4"
                    )
            ...
            with ui.column().classes("flex-1 space-y-4"):
                ...
                with ui.card().classes("p-4 bg-white shadow-sm rounded-lg"):
                    ui.label("Производительность по стратегиям").classes(
                        "text-lg font-semibold text-gray-800 mb-4"
                    )
                ...
                with ui.card().classes("p-4 bg-white shadow-sm rounded-lg"):
                    ui.label("Активность по источникам").classes(
                        "text-lg font-semibold text-gray-800 mb-4"
                    )
            ...
            try:
                ...
                with ui.dialog() as dialog, ui.card().classes("p-6 w-96"):
                    ...
                    with ui.column().classes("space-y-3"):
                        ...
                        ui.label(
                            f"Общий объем: {summary['total_volume']:.2f} {CURRENCY}"
                        )
                        ui.label(
                            f"Средний размер сделки: {summary['avg_transaction_size']:.2f} {CURRENCY}"
                        )
